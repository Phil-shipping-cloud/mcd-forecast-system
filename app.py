...
def load_v2_data():
    ...
    if file_path and os.path.exists(file_path):
        ...
        # 讀取匯率 (Row 0 D欄: 人民幣, Row 1 D欄: 台幣/美金)
        ...
        # 動態判定起算年份 (若 Row 3 填 2025 但資料實為 2026起，自動精準校正)
        raw_year = "2025"
        time_cols = []
        cols_mapping = {}
        ...
        for idx in range(data_start_col, len(row5)):
            ...
            if '月' in r5_val and not any(k in r5_val for k in ['Total', 'TTL', '小計', '合計']):
                ...
                if mo_num:
                    temp_parsed.append((idx, raw_year, int(mo_num)))

        # 💡 若解析出來的年份包含 2025，且使用者確認只有 2026 後數據，自動過濾掉 2025 舊年份或校正起算點
        # 保留 2026 年起的月份 (或根據實際欄位自動校正)
        for idx, yr, mo in temp_parsed:
            col_key = f"{yr}-{mo:02d}"
...
# 💡 過濾掉 2025 年不應出現的歷史欄位 (確保走勢圖從 2026-01 開始)
time_cols = [c for c in time_cols if not c.startswith("2025-")]
